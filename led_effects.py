...
class Cube(_ColoredEffect):
    ...
    The strip is treated as `lines` parallel paths, each `leds_per_line` long.
    A single line is physically routed bottom edge → vertical edge → top edge,
    so per-line offsets are:
    ...
    The bottom face stays fully lit. All four vertical columns rise from 0 to
    full height together, in sync with the audio level. The top face stays
    dark. Lit LEDs take their color from the same palette/color resolution as
    the other effects (`_resolve_colors`).
    """
    def __init__(self, num_leds, color_mode="palette_linear", color=(255, 255, 255),
                 lines=4, leds_per_line=273,
                 bottom_leds=91, vertical_leds=91, top_leds=91):
